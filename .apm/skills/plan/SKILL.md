...
name: plan
description: |
  Plan Mode を起動して Requirements → Design → Implementation の 3 段階で構造化された実装計画を策定し、`exit_plan_mode` で提示してユーザーの明示承認を待つ。新機能開発・複雑なリファクタリング・システム設計など 3 ステップ以上の実装タスクで、コードを書く前に詳細な戦略を立てたいときに使う。
---
