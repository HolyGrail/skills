...
name: tech-debt
description: |
  プロジェクトの技術的負債を TODO/FIXME・依存関係・コード品質・セキュリティ・パフォーマンス・テストカバレッジの観点で言語/フレームワーク自動検出付きで分析し、影響度・修正コスト・ビジネスリスク・ROI を踏まえた優先順位付き改善計画を作成する。レガシー資産のモダナイゼーションや段階的リファクタリング戦略を立てたいときに使う。
---
