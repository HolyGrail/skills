...
compatibility: Designed for Claude Code with Task tool dispatch (subagent spawning). Does not apply when Task tool is unavailable.
---
...
4. **両面評価**: 戻ってきた結果から次を記録する。
   ...
   - **指示側の計測**（判定規則は本節で一元定義、他箇所は本節を参照する）:
     ...
     - **失敗時は「どの [critical] 項目が落ちたか」を提示フォーマットの "不明瞭点" 節に `[critical] 項目 N が × — <落ちた理由 1 行>` の形式で 1 行添える**（原因追跡のため）
   - **追加の自己申告**（subagent のレポート本文から抽出。指示側では測れない）:
     - 再試行回数（subagent が同じ判断をやり直した回数）
...
7. **収束判定**: 連続 2 イテレーションで「新規の不明瞭点 0 件」**かつ**「メトリクス改善が閾値以下（詳細は §反復の打ち切り基準）」を **両方とも** 満たすときに停止（AND 条件）。重要度が高いプロンプトは 3 連続にする。
...
下記は別配布元の skill / 概念への参照であり、本リポジトリには同梱されない。利用環境に存在する場合のみ有効。
