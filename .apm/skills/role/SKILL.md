...
name: role
description: |
  Claude を security / performance / analyzer / frontend / reviewer / architect / qa / mobile などの専門ロール（Evidence-First アプローチ、OWASP・Core Web Vitals・WCAG 等の公式準拠）に切り替え、振る舞い・分析手法・報告形式を専門特化させる。`--agent` オプションでサブエージェント実行による独立コンテキストでの大規模分析も可能。セキュリティ監査・性能解析・根本原因究明など特定領域の専門観点でレビュー/分析したいときに使う。
---
