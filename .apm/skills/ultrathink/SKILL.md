...
name: ultrathink
description: |
  問題の分解 → MECE 分析 → 技術/ビジネス/ユーザーの複数視点検討 → 重要判断ポイントでの対話的確認 → 根拠付き提案、という構造化された深い思考プロセスを段階的に実行する。アーキテクチャ選定（マイクロサービス vs モノリス）・技術選定・大規模トレードオフ分析・レガシーモダナイゼーション戦略など、即答できない重大な判断をじっくり考えたいときに使う。
---
