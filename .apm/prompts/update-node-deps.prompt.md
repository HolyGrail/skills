---
description: |
  Node.js プロジェクトの `package.json` と `npm outdated` を分析し、各パッケージのパッチ/マイナー/メジャー更新を安全（緑）・注意（黄）・危険（赤）で評価して破壊的変更・必要なコード修正・更新計画を提示する。依存関係を最新化したいとき、特定パッケージ更新の影響を事前評価したいとき、TypeScript の型定義整合性も含めて段階的にアップデートしたいときに使う。
---
