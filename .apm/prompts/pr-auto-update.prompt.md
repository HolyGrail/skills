---
description: |
  Git の変更内容を分析して既存 PR の説明文（`.github/PULL_REQUEST_TEMPLATE.md` 構造と HTML コメントを完全保持）と最大 3 つのラベル（`.github/labels.yml` または GitHub API）を自動更新する。実装後に PR 説明・ラベルをまとめて整えたいとき、または `--dry-run` で内容を事前確認したいときに使う。
---
