---
description: |
  PR レビューコメントを must / imo / nits / q の 4 段階で分類し、エラー分析 3 段階アプローチ（情報収集 → 根本原因 → 解決策）で対応計画を作成して優先度順に修正・回帰テスト・進捗報告まで実施する。レビュー指摘や CI 失敗への対応中に、優先度判定と返信テンプレを含めて体系的に対応したいときに使う。
---
