---
description: |
  `git diff --staged` を分析し、CommitLint 設定や git log を踏まえた Conventional Commits 形式のコミットメッセージを生成してクリップボードへコピーする（git コマンドは実行しない）。ステージ済みの変更に対してプロジェクト規約に沿った 1 行コミットメッセージを得たいときに使う。
---
