---
description: |
  `gh issue list` で現在のリポジトリのオープン Issue を取得し、ラベル（bug/security/enhancement 等）と担当者から優先度（高/中/低）別に最大 30 件を 2 行概要付きで一覧表示する。リポジトリの未対応 Issue の全体像を把握したいときや、優先順位付きで取り組み対象を選びたいときに使う。
---
